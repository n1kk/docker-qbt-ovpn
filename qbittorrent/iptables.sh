...
if [[ $iptable_mangle_exit_code == 0 ]]; then
  ...
  echo "8080    socks5" >>/etc/iproute2/rt_tables
  ip rule add fwmark 2 table socks5
  ip route add default via ${DEFAULT_GATEWAY} table socks5
...
# accept input to dante socks proxy
iptables -A INPUT -i eth0 -p tcp --dport 1080 -j ACCEPT
iptables -A INPUT -i eth0 -p tcp --sport 1080 -j ACCEPT
...
if [[ $iptable_mangle_exit_code == 0 ]]; then
  ...
  iptables -t mangle -A OUTPUT -p tcp --dport 1080 -j MARK --set-mark 2
  iptables -t mangle -A OUTPUT -p tcp --sport 1080 -j MARK --set-mark 2
fi
...
iptables -A OUTPUT -o eth0 -p tcp --dport 1080 -j ACCEPT
